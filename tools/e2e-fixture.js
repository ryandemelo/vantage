...
const PROMPTS = [
  ...
  {
    text: 'Draft an email to jane.tan@agency.gov.sg about CASE-104477, her NRIC is S1234567D',
    want: { workType: 'drafting', redactions: ['email', 'nric'] }
  },
  {
...
(async () => {
  ...
  if (events.length) {
    ...
    if (sensitive) {
      ...
      check('NRIC redacted in stored text', sensitive.promptText.indexOf('S1234567D') === -1, sensitive.promptText);
      check('redaction counted', sensitive.redactionCount >= 2, String(sensitive.redactionCount));
      check('redaction types recorded', !!sensitive.redactionHits.email && !!sensitive.redactionHits.nric,
