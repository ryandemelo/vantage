...
console.log('\nredaction');
{
  ...
  const r1 = VG.redact('Email jane.tan@agency.gov.sg about CASE-104477 for Project Kingfisher', s);
  ...
  const r2 = VG.redact('My NRIC is S1234567D and card 4242 4242 4242 4242', s);
...
console.log('\nclassification');
{
  ...
  const cases = [
    ...
    ['Reply to this enquiry from a member of the public about their appeal', 'citizen'],
...
console.log('\nclassifier logic');
{
  ...
  // "policy" collides badly with engineering vocabulary in a govtech.
  ...
  const holiday = VG.classify('Draft the agency circular on public holiday arrangements for the division', s);
  check('agency vocabulary beats personal', holiday.id !== 'personal', holiday.id);
...
console.log('\naccount tier');
{
  ...
  const minimal = Object.assign({}, VG.DEFAULT_SETTINGS, { corporateDomains: ['agency.gov.sg'] });
  ...
  const r1 = VG.detectAccount(adapter, spyDoc({ '#email': 'jane.tan@agency.gov.sg' }), minimal);
  ...
  check('corporate domain -> enterprise',
    VG.detectAccount(adapter, doc({ '#email': 'jane.tan@agency.gov.sg' }), standard) === 'enterprise', '');
  check('subdomain of corporate domain -> enterprise',
    VG.detectAccount(adapter, doc({ '#email': 'j@ict.agency.gov.sg' }), standard) === 'enterprise', '');
...
{
  const s = Object.assign({}, VG.DEFAULT_SETTINGS, {
    policyAdapters: [{ id: 'internal', label: 'Agency Assistant', hosts: ['ai.agency.gov.sg'], revision: 4 }],
    customAdapters: [
      { id: 'internal', label: 'User copy that must lose', hosts: ['ai.agency.gov.sg'] },
      { id: 'mine', label: 'My tool', hosts: ['tool.example'] }
    ]
  });
  const a = VG.resolveAdapter('ai.agency.gov.sg', s);
  check('policy wins over user for same id', a && a.label === 'Agency Assistant', a && a.label);
...
console.log('\nscheduled upload');
{
  const base = Object.assign({}, VG.DEFAULT_SETTINGS, {
    uploadEnabled: true, uploadUrl: 'https://collector.agency.gov.sg/v1/vantage',
...
async function uploadPayloadChecks() {
  ...
  const mkBody = async (over) => {
    ...
    return VG.buildUploadPayload({ report: rep, events, settings: st, org: { userKey: 'dev1', agency: 'MOM' }, period });
  };
  ...
  check('payload carries the device and agency', agg.device.key === 'dev1' && agg.device.agency === 'MOM', '');
  ...
  const desc = VG.uploadDescription(Object.assign({}, VG.DEFAULT_SETTINGS, {
    uploadEnabled: true, uploadUrl: 'https://collector.agency.gov.sg/v1', uploadCadence: 'weekly'
  }));
  check('transparency panel names the host and cadence',
    desc.host === 'collector.agency.gov.sg' && desc.when === 'once a week', JSON.stringify(desc));
...
(async () => {
  ...
  const customSettings = Object.assign({}, s, {
    policyAdapters: [{ id: 'internal', label: 'Agency Assistant', hosts: ['ai.agency.gov.sg'] }]
  });
  const customEvents = events.concat([
    Object.assign(VG.newEvent(), {
      ts: now - day, day: VG.localDay(now - day), site: 'policy:internal',
      host: 'ai.agency.gov.sg', conversationHash: 'c9', turn: 1,
