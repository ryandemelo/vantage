...
| Redaction hits by type | Counts only, for example `{email: 2, nric: 1}` |
...
`drafting` `comms` `policy` `comprehension` `translation` `coding` `data` `procurement` `citizen` `research` `media` `learning` `admin` `hr` `appraisal` `legal` `personal` `other`
...
```json
{ "policy": [["cabinet paper", 6], ["ministerial brief", 6]],
  "procurement": [["gebiz", 6], ["ITQ", 5]] }
```
...
2. Anti patterns subtract. "IAM policy" and "bucket policy" are engineering rather than policy work. "Smart contract" is not procurement. Anything containing a code fence is not personal. These collisions are the main source of false positives in a government vocabulary and they are handled explicitly.
...
| --- | --- | --- |
| Pushed value | `orgAgency`, `orgDivision` and `orgCohort` in policy | Your push can target groups |
| Domain lookup | One `agencyDomainMap` pushed identically to everyone. Each device resolves its own agency from the domain of the signed in browser profile | Your push goes uniformly to all agencies |
...
Division below agency is not derivable from a domain. Either target it in the push or leave it blank.
...
HKLM\Software\Policies\Microsoft\Edge\ExtensionSettings
  {"nnnkddpplabnomlnaolmicnanjphpibp": {"runtime_allowed_hosts": ["*://collector.your-domain.gov"]}}
```
...
```json
{
  ...
  "extraTaxonomyTerms": { "policy": [{ "term": "cabinet paper", "weight": 6 }] },
  "agencyDomainMap": { "mom.gov.sg": "Ministry of Manpower" },
  "deriveIdentity": true,
  "reportEmailTo": "ai-programme@agency.gov.sg",
  "weekStartsOn": 1,
...
```
node tools/build.js
CODEBASE=https://intranet.your-domain.gov.sg/vantage node tools/build.js
```
