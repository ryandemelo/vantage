...
const handlers = {
  ...
  /* ------------------------- org attribution ------------------------- *
   * Zero manual entry. Business unit comes from either a pushed policy value or,
   * where the optional identity permission is granted, from the DOMAIN of the
   * signed-in browser profile mapped through a pushed domain->business unit table.
  ...
  async GET_ORG() {
    const settings = await VG.settings.get();
    const out = {
      businessUnit: settings.orgUnit || '',
      division: settings.orgDivision || '',
      cohort: settings.orgCohort || '',
      source: settings.orgUnit ? 'policy' : 'none',
      userKey: '',
      domain: ''
    };
    ...
    const map = settings.orgDomainMap || {};
    const hit = Object.keys(map).find((d) => domain === d || domain.endsWith('.' + d));
    if (hit && !out.businessUnit) { out.businessUnit = map[hit]; out.source = 'domain'; }
    return out;
  },
